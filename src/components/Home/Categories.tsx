                    ...
                    categories.map((category: Meal) => {
                        return (
                            <SwiperSlide style={{
                                ...
                                alignItems: 'end',
                                // position: screenSize.width >= 1024 ? 'relative' : 'initial',
                                // left: screenSize.width >= 1024 ? 25 + '%' : undefined
                            }}>
                                <div
                                    className="flex flex-col justify-center items-center cursor-pointer small-laptop:pt-[30%] small-laptop:pl-[25%]"
                                    onClick={() => setActiveCategory(category._id)}
                                >
                                    <div className="h-[5rem] w-[5rem] flex justify-center items-center">
                                        <img src={category.poster_image} className="h-full w-full" alt="None" />
                                    </div>
                                    <p className="font-semibold relative bottom-[1rem] large-phone:pt-[2%] tablet:text-xl">{category.name}</p>
                                </div>
                            </SwiperSlide>
                        )
                    })
